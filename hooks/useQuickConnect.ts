...
const messages = getSeerrMessages('hooks.useQuickConnect');
...
export const useQuickConnect = ({
  ...
  const startPolling = useCallback(
      ...
      pollingInterval.current = setInterval(async () => {
        try {
          const response = await axios.get(
            serverUrl + '/api/v1/auth/jellyfin/quickconnect/check',
            {
              params: { secret },
            }
          );
    ...
    [authenticateWithQuickConnect, intl, onError, serverUrl]
  );
  ...
  const initiateQuickConnect = useCallback(async () => {
    ...
    try {
      const response = await axios.post(
        serverUrl + '/api/v1/auth/jellyfin/quickconnect/initiate'
      );
  ...
  }, [startPolling, onError, intl, serverUrl]);
